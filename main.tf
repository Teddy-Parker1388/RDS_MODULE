locals {
  ...
  option_group                = var.create_db_option ? aws_db_parameter_group.db_opt_grp[0].name : var.db_option_group_name_ref

  create_rds_cluster = can(regex("aurora", "mysql")) || var.create_rds_cluster ? 1 : 0
}
